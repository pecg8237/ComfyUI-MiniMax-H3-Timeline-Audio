...
class AudioTimelineTests(unittest.TestCase):
    def test_slice_includes_context_and_h3_padding(self):
        ...
        continuation = Segment(1, 124, 39, 124, 85)
        continuation_slice = slice_timeline_audio(audio, continuation)
        self.assertEqual(continuation_slice["waveform"].shape, (1, 1, 124))
        self.assertEqual(continuation_slice["waveform"][0, 0, 0].item(), 86)
        self.assertEqual(continuation_slice["waveform"][0, 0, -1].item(), 209)
