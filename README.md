...
`MiniMax H3 Long Timeline Audio Sampler` combines the reference inputs from ComfyUI's built-in `MiniMax H3 Reference to Video` node with custom sampler inputs. It splits a long 24 fps timeline into model-sized AV latent segments, carries 22 or 39 frames of sampled video and audio latent into the next segment, saves every segment to SSD, and decodes the selected checkpoints to one MP4 without retaining the full decoded movie in RAM. Its generation path follows the upstream author's Aug. 25 continuation behavior: the carried AV context is supplied as a guide, counts inside the strict raw-segment limit, and is removed unchanged during assembly. No latent locking or decoded-frame crossfade is applied.

Prompts may use the official base-mode `integrated_multimodal_description:`, the full-reference Ref2VA `detailed_description:`, or a plain `[Shot 1] ... [Shot N] At MM:SS.mmm, ...` timeline. Each Shot starts in the segment containing its global timestamp. Continuation prompts follow the author's Aug. 25 clock: the removable AV guide occupies the first 22 or 39 frames of the local segment timeline, and new Shot timestamps are rebased after that guide. An ongoing Shot is not pasted in full at every boundary, which avoids asking H3 to replay its opening action.
...
A segment with no new Shot receives the preceding AV guide plus a concise forward-continuation instruction rather than a repeated copy of the active Shot.
...
`ref_audio_0` is also the final master soundtrack. The node trims it to the exact delivered video duration, pads a short source with silence, converts mono to stereo when needed, and muxes it directly into `master.mp4`. Generated H3 audio remains inside the segment latents for AV continuation and lip-motion guidance, but it is not used in the final MP4. At least `ref_audio_0` must be connected.

`max_raw_frames` is a strict H3 `17k+5` raw-segment ceiling, matching the upstream author's Aug. 25 implementation. The carried guide is included inside that ceiling. For example, `max_raw_frames=124` with `context_frames=39` allows 85 new delivered frames in each full continuation segment; it does not generate a 175-frame raw segment to force a five-second delivered window. `length` is the exact delivered frame count, so use `720` for 30 seconds at 24 fps.
...
For continuation segments, the renderer relies on the preceding AV guide and does not repeat the full prose of an already active Shot. Shared subject definitions, `[Global Instructions]`, soundscape, and music constraints remain available to every segment. An outer Markdown `text` code fence is stripped automatically before encoding. Ambiguous bare `S1 = ...` definitions are rejected before generation: visual references must use `<Subject N>`, while `(Sx)` is reserved for speakers.
