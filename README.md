...
`MiniMax H3 Long Timeline Audio Sampler` combines the reference inputs from ComfyUI's built-in `MiniMax H3 Reference to Video` node with custom sampler inputs. It splits a long 24 fps timeline into model-sized AV latent segments, carries 22 or 39 frames of sampled video and audio latent into the next segment, saves every segment to SSD, and decodes the selected checkpoints to one MP4 without retaining the full decoded movie in RAM. The carried AV context is supplied as a frame-zero guide. During final assembly, the timeline node blends the final four delivered frames before each boundary toward the next segment's first delivered frame, while preserving the total frame count. This remains effective even when the model itself changes abruptly between its decoded context and new output. It does not hard-mask the new latent head. The upstream generic long sampler remains guide-only without decoded-frame blending.
